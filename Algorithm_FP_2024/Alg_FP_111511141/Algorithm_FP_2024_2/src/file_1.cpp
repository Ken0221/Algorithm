   ...
   private:
    ...
    void twoWayPartition() {
        ...
        if (numAgents <= 25) {
            ...
            int maxMemmory = -1;
            vector<bool> agentMemoryValid(numAgents, true);
            int max_i = -1;
            int nextServer = 0;
            ...
            for (int n = 0; n < numAgents; n++) {
                ...
                for (int i = 0; i < numAgents; i++) {
                    if (agentMemoryValid[i]) {
                        if (agentMemory[i] >= maxMemmory) {
                            maxMemmory = agentMemory[i];
                            max_i = i;
                        }
                    }
                    // sum += agentMemoryValid[i];
                }
                // cout << "numAgents" << numAgents << endl;
                // cout << "sum(agentMemoryValid) = " << sum << endl;
                // if (maxMemmory == -1) {
                //     cout << "no more agent, error" << endl;
                // }
                // cout << n + 1 << " maxMemmory = " << maxMemmory << endl;
                agentMemoryValid[max_i] = false;
                if (serverLoads[nextServer] + maxMemmory <= maxDiskCapacity) {
                    serverLoads[nextServer] += maxMemmory;
                    serverAssignments[max_i] = nextServer;
                    nextServer = 1 - nextServer;
                } else if (serverLoads[1 - nextServer] + maxMemmory <=
                           maxDiskCapacity) {
                    serverLoads[1 - nextServer] += maxMemmory;
                    serverAssignments[max_i] = 1 - nextServer;
                } else {
                    cout << "error" << endl;
                }
            }
